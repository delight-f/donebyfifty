...
- **30% minimum rate floor:** `max(marginal_rate, 0.30)` per owner, weighted by ownership share, correctly preventing high earners from deferring gains into low-income years
...
BridgeCheck generates multi-asset correlated returns using Cholesky decomposition, the standard approach in financial engineering for producing correlated Gaussian draws from independent normals. The correlation matrix is factorised once as $L L^T$, and correlated z-scores are obtained as $L \cdot \vec{z}$ where $\vec{z}$ is a vector of independent standard normals.
...
$$ \ln(r_{t+1}) = \ln(\theta) + \phi(\ln(r_t) - \ln(\theta)) + \sigma_\epsilon \cdot \epsilon_t $$

where $\phi = e^{-\kappa}$ is the exact AR(1) coefficient (mean-reversion strength $\kappa$, default 0.20/year, half-life ≈ 3.5 years), and $\sigma_\epsilon = \tilde{\sigma} \sqrt{(1 - \phi^2) / (2\kappa)}$ is the discrete shock volatility calibrated to produce a stationary distribution consistent with the continuous-time parameter $\tilde{\sigma}$.
...
Prior versions of the code incorrectly applied both CPI indexation AND the 50% CGT discount, computing `max(marginal_rate × 0.5, 0.30)`. The Treasury reform *replaces* the discount with indexation — they do not stack. The corrected formula is `max(marginal_rate, 0.30)`, which means earners in the 37% and 45% brackets pay at their full marginal rate on real gains. The model was corrected after a line-by-line audit confirmed the error against the policy text.
