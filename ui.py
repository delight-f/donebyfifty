...
def display_results(results: SimulationResults, household: Household | None = None, start_age: int = 37, success_threshold: float = 0.95) -> None:
    ...
    note_lines: list[str] = [
        "[bold]Valuation basis:[/] Bridge assets in today's dollars"
        " (deflated). Super and mortgage figures in nominal (then-year) dollars.",
        f"[bold]Simulation scope:[/] Bridge period only (to age {results.horizon_age})."
        f" Post-bridge retirement, Age Pension, and minimum pension rules not modelled.",
    ]
    if results.remaining_mortgage_p50 and any(
        ...
        if has_stochastic:
            ...
            note_lines.append(
                f"[bold]Mortgage rates:[/] Stochastic (Black-Karasinski). "
                f"Long-run mean {theta*100:.1f}%, volatility {sigma_mod:.0f}%/"
                f"\u221ayr (stress {sigma_str:.0f}%/\u221ayr), "
                f"mean-reversion {kappa:.2f}/yr, equity corr {rho:.2f}. "
                f"Steady-state ~95% interval [{lo:.1f}%, {hi:.1f}%]."
            )
        else:
            note_lines.append(
                f"[bold]Mortgage rates:[/] Fixed for full simulation. "
                f"Real rate volatility \u2014 including rising rates while offset "
                f"is drawn down \u2014 is not captured. This may understate "
                f"repayment stress in rising-rate environments."
            )
